// admin-cuestionario.js - carga y renderiza el listado de preguntas
// admin-cuestionario.js - carga y renderiza el listado de preguntas y permite añadir nuevas
(function(){
  // Añadir botón y formulario para nueva pregunta
  document.addEventListener('DOMContentLoaded', function(){
    const container = document.getElementById('questionsList');
    if(!container) return;
    let addBtn = document.getElementById('addPreguntaBtn');
    if(!addBtn){
      addBtn = document.createElement('button');
      addBtn.id = 'addPreguntaBtn';
  ...
  function showAddForm(){
    ...
    (function(){
      ...
      function showAddForm(){
        ...
        idInp.type = 'text';
        idInp.className = 'form-input';
        idInp.placeholder = 'ID único (ej: preguntaNueva)';

        const titleInp = document.createElement('input');
        titleInp.type = 'text';
        titleInp.className = 'form-input';
        titleInp.placeholder = 'Texto de la pregunta';

        const tipoSel = document.createElement('select');
        tipoSel.className = 'form-select';
        ['select','checkbox','text'].forEach(t => {
        ...
        btnAddOption.className = 'btn btn-secondary';
        btnAddOption.textContent = 'Añadir opción';
        btnAddOption.onclick = (e) => { e.preventDefault(); optsContainer.appendChild(crearInputOption()); };
        ...
        btnSave.onclick = async (e) => {
          ...
          if(!id || !titulo) return alert('ID y texto requeridos');
          let options = [];
          if(tipo !== 'text'){
            const rows = Array.from(optsContainer.querySelectorAll('.option-edit-row'));
            options = rows.map(r => {
              const inputs = r.querySelectorAll('input');
              return { value: inputs[0].value || '', label: inputs[1].value || inputs[0].value || '' };
            }).filter(o => o.value || o.label);
            if(options.length === 0) return alert('Agrega al menos una opción');
          }
        ...
        btnCancel.onclick = (e) => { e.preventDefault(); form.remove(); };

        form.appendChild(createLabel('ID único')); form.appendChild(idInp);
        form.appendChild(createLabel('Texto de la pregunta')); form.appendChild(titleInp);
        form.appendChild(createLabel('Tipo de pregunta')); form.appendChild(tipoSel);
        form.appendChild(createLabel('Opciones (si aplica)')); form.appendChild(optsContainer);
        form.appendChild(btnAddOption);
      ...
      function createLabel(text){ const l = document.createElement('div'); l.style.fontWeight='600'; l.style.marginTop='0.5rem'; l.textContent = text; return l; }
      // Usa la API para obtener/editar/eliminar preguntas
      ...
      function crearCartaPregunta(p){
        ...
        meta.textContent = 'Tipo: ' + (p.tipo === 'checkbox' ? 'Selección múltiple' : 'Selección única');
      ...
      function enterEditMode(card, p){
        ...
        titleInp.type = 'text';
        titleInp.className = 'form-input';
        titleInp.value = p.titulo || '';

        const tipoSel = document.createElement('select');
        tipoSel.className = 'form-select';
        ['select','checkbox','text'].forEach(t => {
          const o = document.createElement('option'); o.value = t; o.textContent = t; if(p.tipo===t) o.selected = true; tipoSel.appendChild(o);
        });
        ...
        if(Array.isArray(p.options)){
          p.options.forEach(o => {
            optsContainer.appendChild(crearInputOption(o.value, o.label));
          });
        }
        ...
        btnSave.className = 'btn btn-primary';
        btnSave.textContent = 'Guardar';
        btnSave.onclick = (e) => { e.preventDefault(); savePregunta(id, titleInp.value, tipoSel.value, optsContainer); };

        const btnCancel = document.createElement('button');
        btnCancel.className = 'btn btn-secondary';
        btnCancel.textContent = 'Cancelar';
        btnCancel.onclick = (e) => { e.preventDefault(); renderAll(); };
      ...
      async function savePregunta(id, titulo, tipo, optsContainer){
        const optionRows = Array.from(optsContainer.querySelectorAll('.option-edit-row'));
      ...
      async function deletePregunta(id, card){
        ...
        if(data.success){
          card.remove();
          // también actualizar la página de evaluación (si está abierta) recargando datos
        } else {
      ...
      async function renderAll(){
        ...
        preguntas.forEach(p => container.appendChild(crearCartaPregunta(p)));
      }
